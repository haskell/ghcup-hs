...
setToolVersion' tool tver mTmpDir = do
  ...
  case vspec of
    ...
    VRight spec -> do
      ...
      unless (null shadows) $ logWarn $ T.pack $ prettyHFError (ToolShadowed tool (_tvVersion tver) shadows)
  ...
  when (isNothing mTmpDir) $ do
    ...
    supportedGHC <- lift $ maybe (pure []) (getHLSGHCs . _vrVersion . fst) currentHLS
    lift $ warnAboutHlsCompatibility (_vrVersion . fst <$> currentHLS) (_vrVersion . fst <$> currentGHC) supportedGHC
