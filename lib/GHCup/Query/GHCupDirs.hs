...
import GHCup.Prelude.Logger.Internal ( logDebug, logWarn )
...
-- | The install dir for a tool is simply determined by its tool name
-- and version.
...
toolInstallDestination tool ver = do
  ...
  pure (bDir `appendGHCupPath` prettyShow ver)
